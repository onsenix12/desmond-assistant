        ...
        <div className="space-y-3">
          {selectedDayEvents.map((event) => {
            ...
            return (
              <div 
                key={event.id}
                className={`p-3 rounded-lg border-l-4 border-${colorName}-500 ${eventColorClasses}`}
              >
                <div className="flex items-start justify-between">
                  ...
                  <span className={`text-xs px-2 py-1 rounded bg-${colorName}-200 text-${colorName}-800`}>
                    {event.type}
                  </span>
                </div>
              </div>
